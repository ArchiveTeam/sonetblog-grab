...
LUA_SHA1 = get_hash(os.path.join(CWD, 'sonetblog.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--connect-timeout', '1',
            '--tries', 'inf',
            '--domains', 's-net.ne.jp,ss-blog.jp',
            '--span-hosts',
            '--waitretry', '30',
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'blog':
                wget_args.extend(['--warc-header', 'sonetblog-blog: '+item_value])
                wget_args.append('https://{}.blog.ss-blog.jp/'.format(item_value))
            elif item_type == 'archive':
                blog, post = item_value.split(':')
                wget_args.append('https://{}.blog.ss-blog.jp/archive/{}'.format(blog, post))
            elif item_type == 'tag':
                blog, post = item_value.split(':')
                wget_args.append('https://{}.blog.ss-blog.jp/tag/{}'.format(blog, post))
            elif item_type == 'index':
                blog, post = item_value.split(':')
                wget_args.append('https://{}.blog.ss-blog.jp/index/{}'.format(blog, post))
            elif item_type == 'post':
                blog, post = item_value.split(':')
                wget_args.append('https://{}.blog.ss-blog.jp/{}'.format(blog, post))
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'sonetblog-asset: '+url])
